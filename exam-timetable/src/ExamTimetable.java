...
public class ExamTimetable {
    ...
    /*private static Node checkGraphAndAdd(List<Course> courseList, int i, List<Course> myCourses) {
        Node node = null;
        ...
        if (!myCourses.get(i).isAddedToGraph()) {
            ...
            graph.add(node);
            myCourses.get(i).setAddedToGraph(true);
            System.out.println("Node added in Graph "+node.getName());
        }
        else{
            for (Node n: graph) {
                if (myCourses.get(i).getValue().equals(n.getName())) {
                    assert false;
                    node.setName(n.getName());
                    node.setColor(n.getColor());
                    node.setColored(n.isColored());
                    node.setNeighbours(n.getNeighbours());
...
*/
    ...
    /*public static List<Node> initialize(String courseFileName, String studentFileName){
        ...
        for (Student student:
            ...
            for (int i = 0; i < myCourses.size()-1; i++) {
                Node mainNode = ExamTimetable.checkGraphAndAdd(courseList, i, myCourses);
                mainNode.setColored(false);
                mainNode.setColor(-1);
                for (int j = 1; j < myCourses.size(); j++) {
                    Node subNode = ExamTimetable.checkGraphAndAdd(courseList, j, myCourses);
...
*/
    ...
    /*public static void colorGraph(List<Node> graph){
        for (Node n: graph) {
            n.setColored(false);
            n.setColor(-1);
        }
        ...
        int count=0;
        for (boolean availableColor : availableColors) {
